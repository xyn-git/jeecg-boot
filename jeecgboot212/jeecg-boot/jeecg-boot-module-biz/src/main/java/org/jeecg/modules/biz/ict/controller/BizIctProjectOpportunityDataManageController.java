 ...
 */
@RestController
@RequestMapping("/ict/bizIctProjectOpportunityDataManage")
@Slf4j
public class BizIctProjectOpportunityDataManageController extends JeecgController<BizIctProjectOpportunity, IBizIctProjectOpportunityService> {
	@Autowired
	...
	@PermissionData(pageComponent = "ict/BizIctProjectOpportunityDataManageList")
	...
	public Result<?> add(@RequestBody BizIctProjectOpportunity bizIctProjectOpportunity) {
		...
		//log.info("User="+sysUser.getUsername() +"  name="+sysUser.getRealname()+"  teamvalue="+team_item_value +"  orgName="+sysDepart.getDepartName());
